...
ggplot(PeterPaul.chem.nutrients, aes(x=factor(PeterPaul.chem.nutrients$month, levels=1:12, labels=month.abb))) + geom_bar()

#Alter to include missing months

ggplot(PeterPaul.chem.nutrients, aes(x=factor(PeterPaul.chem.nutrients$month, levels=1:12, labels=month.abb))) + geom_bar() + scale_x_discrete(name="month", drop=FALSE)
...
plot_1 <- PeterPaul.chem.nutrients %>% filter(depth == 0) %>% 
  ggplot(aes(x=daynum, y=temperature_C, color=year4)) + geom_point() + 
  ylab("surface temperature (C)") +     #can also do labs(y=" ")
  facet_wrap(vars(lakename), nrow = 2)

plot_1

#2. 
# Plot temperature (y-axis) by date (x-axis). Color your points by depth.
...
plot_2 <- PeterPaul.chem.nutrients %>%
...
plot_3 <- EPAair %>% ggplot(aes(x=PM2.5, y=Ozone, color=mean_Lat)) +
