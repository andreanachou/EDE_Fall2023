...
plot1 <- EPAair %>% ggplot(aes(Ozone, color=Year, fill=Year)) + 
  geom_density(alpha=0.5) +
  scale_fill_manual(values=c("lightgreen", "lightpink")) +
  labs(x="Ozone AQI value", y="Density")
plot1

plot2 <- EPAair %>% ggplot(aes(x=Year, y=Ozone)) + geom_boxplot() + 
  stat_summary(geom="text", fun=max, vjust=-1, size=4, label=c("a", "b")) + 
  labs(x="Year", y="Ozone AQI Value") + 
  ylim(0, 150)
plot2

plot3 <- EPAair %>% ggplot(aes(x=Year, y=Ozone)) + 
  geom_violin(draw_quantiles = 0.5) +
  stat_summary(geom="text", fun=max, vjust=-1, size=4, label="max") +
  labs(x="Year", y="Ozone AQI Value") +
  ylim(0, 150)
plot3  
```
...
regression2 <- lm(data=EPAair, PM2.5 ~ Ozone)
summary(regression2)

#The p-value is essentially zero, the R-squared value is 0.1314.
...
scatter3 <- EPAair %>% ggplot(aes(x=Ozone, y=PM2.5)) + geom_point() +
...
library(corrplot)
library(dplyr)
EPAair.subset <- EPAair %>% select(mean_Lat:Ozone) %>% na.omit()
