...
> Answer: Matrices and dataframes show up as data, while vectors and single-value variables show up as values. 

Question: In the R chunk below, write "dataframe1\$". Press `tab` after you type the dollar sign. What happens?

> Answer: R will suggest all the columns in the dataframe1
...
```{r DataExercise-1a}
high_f <- c(81, 83, 83, 84, 78, 71, 79, 82, 85, 84)
low_f <- c(81, 83, 83, 84, 78, 71, 79, 82, 85, 84)
...
```{r DataExercise-1c}
durham_forecast <- data.frame(high_f, low_f, high_c, low_c)
#or use data.frame("High (F)" = high_f, "Low (F)" = low_f, ...)

names(durham_forecast) <- c("High (F)", "Low (F)", "High (C)", "Low (C)")

view(durham_forecast)
```
...
```{r DataExercise-1d}
summary(durham_forecast)
sd(durham_forecast$`High (F)`)
```
