...
sum(is.na(BR_complete))
summary(BR_complete$Rainfall)

#jan 1901 to dec 2016 month frequency -> left join by date sequence ->  
...
rain_plot <- BR_complete %>% ggplot(aes(x = Date, y = Rainfall)) + geom_line() +
  geom_smooth(method="lm")
rain_plot

temp_plot <- BR_complete %>% ggplot(aes(x = Date, y = Temperature_C)) +
...
BR_rain_ts <- ts(BR_complete$Rainfall, start=c(1901,1,1), frequency=12)
BR_temp_ts <- ts(BR_complete$Temperature_C, start=c(1901,1,1), frequency=12)
...
BR_rain_df <- as.data.frame(BR_rain_decomp$time.series[, 1:3])
BR_temp_df <- as.data.frame(BR_temp_decomp$time.series[, 1:3])

# Exercise 10: Visualize how the trend maps onto the data for both series

#ggplot(BR_rain_df) + geom_line(aes(x = ))
