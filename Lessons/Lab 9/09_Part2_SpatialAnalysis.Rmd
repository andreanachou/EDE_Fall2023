...
counties_sf<- st_read(here('Data/Spatial/cb_2018_us_county_20m.shp')) %>% 
  dplyr::filter(STATEFP == 37) #Filter for just NC Counties
...
ggplot(huc8_sf) + geom_sf(aes(fill=ACRES)) + scale_fill_viridis_c()
...
huc8_sf_2 <- st_read(here("./Data/Spatial/NCHUC8.shp")) %>%
  dplyr::filter(SUBBASIN == "Upper Neuse")

#Create a map

mapView(huc8_sf, col.regions = "orange") + mapView(huc8_sf_2, col.regions = "purple")

ggplot() + geom_sf(data=huc8_sf, fill="orange") + 
  geom_sf(data=huc8_sf_2, fill="purple") 
...
huc2_sf <- huc8_sf %>%
  group_by(DWQ_Basin) %>%
  summarize(Total_Acres = sum(ACRES), Total_Miles = sum(SQ_MILES))
...
tri_sf <- counties_sf_utm %>% dplyr::filter(NAME %in% c("Chatham", "Durham",
                                                 "Orange", "Wake"))
  
#Grab the intersecting HUC_8s

#tri_sf_intersect <- counties_sf_utm %>% 
  #dplyr::filter(st_intersects(x =., y=neuse_sf, sparse = FALSE))
