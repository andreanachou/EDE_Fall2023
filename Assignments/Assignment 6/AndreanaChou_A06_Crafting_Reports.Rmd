...
```{r table, echo=FALSE}
epa_air <- cbind(c("Source", "Date", "Filename"), 
                 c("EPA Air Quality System (AQS)", "2018-2019",
                   "EPAair_O3_PM25_NC1819_Processed.csv"))

#created a matrix by binding the two vector columns, assigned to object epa_air

knitr::kable(epa_air, align=c("r", "l"), col.names = c("Item", "Value"))

#pulled singular function "kable" from knitr using "::"
#used epa_air matrix, applied different alignment to the columns using vector c()
```
...
```{r project.setup, results="hide", echo=TRUE, message=FALSE, warning=FALSE}
...
#had to use results="hide" instead of include=FALSE to keep output from being
  #shown, echo=TRUE ensures code is displayed
#message=FALSE to hide messages, warning=FALSE to hide errors
```
...
#table for PM25 summary

knitr::kable(summary_Ozone, caption="Mean Ozone", 
             col.names=c("County", "$\\mu g/m^3$"))
#table for Ozone summary
```
...
```{r ozone plots, echo=FALSE, fig.cap = c("Monthly Ozone, 2018", "Monthly Ozone, 2019"), fig.align="left", fig.dim=c(6, 4)}
library(ggplot2)

Ozone_2018_graph <- epa_data %>% filter(Year == "2018") %>% drop_na(Ozone) %>% ggplot(aes(x=factor(Month, levels=1:12, labels=month.abb), y=Ozone)) + geom_boxplot() + labs(x="Month", y=expression(paste("Ozone Concentration ", mu, g/m^3)), title="2018 Recorded Ozone by Month")

#geom_text(y=expression(paste("Ozone Concentration", mug/m^3)), show.label=FALSE)

Ozone_2018_graph

Ozone_2019_graph <- epa_data %>% filter(Year == "2019") %>% drop_na(Ozone) %>% ggplot(aes(x=factor(Month, levels=1:12, labels=month.abb), y=Ozone)) + geom_boxplot() + labs(x="Month", y=expression(paste("Ozone Concentration ", mu, g/m^3)), title="2019 Recorded Ozone by Month")
...
https://github.com/andreanachou/EDE_Fall2023 
